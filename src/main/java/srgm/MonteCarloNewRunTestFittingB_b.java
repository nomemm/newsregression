...
public class MonteCarloNewRunTestFittingB_b {

	public static Map<String, double[]> map = new HashMap<String, double[]>();
	// = new double[] { "tizen", "cyan", "nemo", "mer" , "tatar", "sedmsk", "minfin", "pmo"};
//	public static double[] origpred = new double[] {0.8520723436, 0.7664423, 0.8660391, 0.9169082 , 0.8328125, 0.8717949, 0.5107784, 0.8338462};
//	public static double [] origacc = new double[] {0.003834960321, 0.0033695024, 0.0004679573, 0.0081518495 , 0.0037520477, 0.0069587749, 0.0024458650, 0.0196345152};
	
	public static void main(String[] args) {
		
//		double THRESHOLD = 0.1;
//		boolean paExit = false;
//		boolean accExit = false;
//		
		double SD = 0.5;
		...
		int datarel = Integer.valueOf(args[4]);  
		...
		if (dt.equalsIgnoreCase("mobile")) {
			datasets = new String[] { "tizen", "cyan", "nemo", "mer" , "tatar", "sedmsk", "minfin", "pmo"};
//			datasets = new String[] { "tizen"};
			//datasets = new String[] { "nemo" };
			  releases = new int[] { 8, 4, 16, 13, 8, 8, 8, 8 };
//			  releases = new int[] { 8};
			  
		//	  datasets = new String[] {dataname};
		//	  releases = new int[] { datarel};
		} else {
		...
		for (int counter = 0; counter < datasets.length; counter++) {
			...
			if (dt.equalsIgnoreCase("mobile")) {
				/* config mobile */
				dir = new File("/Users/vladimirivanov/Downloads/monte_carlo/mobile-3/" + SD + "/"+dataset+".csv");
			} else {
				// config - comm
				dir = new File("/Users/vladimirivanov/Downloads/Innopolis/lips/data-analysis/srgm/data/");
			}

			int boot_size = dir.listFiles().length;
			int ii = 0;
			...
			for (File f : dir.listFiles()) {
				...
				for (String s : data) {
					...
					obs.add(Double.parseDouble(s.split(",")[1]), Double.parseDouble(s.split(",")[2]));
				}
...
//							double error = Math.abs(bootpred - origpred[counter]) / origpred[counter];
//							if (error > THRESHOLD)
//							{
//								exceedCount++;
////								System.err.println("PA:" + exceedCount);
//							}
//							
//							double errorAcc = Math.abs(bootacc - origacc[counter]) / origacc[counter];
//							if (errorAcc > THRESHOLD)
//							{
//								exceedCountAcc++;
////								System.err.println("AcFP:" + exceedCountAcc);
//							}
//							
//							
//							//============= //============= //============= //=============
//							if(exceedCount > 5)
//							{
////								System.err.println("! " + SD + "	" + dataset + "	" + exceedCount);
//								paExit = true;
//							}
//							else
//							{
...
//							if(exceedCountAcc > 5)
//							{
////								System.err.println("! " + SD + "	" + dataset + "	" + exceedCountAcc);
//								accExit = true;
//							}
//							else
//							{
