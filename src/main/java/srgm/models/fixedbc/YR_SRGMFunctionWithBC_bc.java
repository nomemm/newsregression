...
public class YR_SRGMFunctionWithBC_bc implements ParametricUnivariateFunction  {

	private int n;

	public YR_SRGMFunctionWithBC_bc(int nval) {
		n = nval;
	}

	public YR_SRGMFunctionWithBC_bc() {
		n = 1;	
	}
...
/////// ============== edit this part
	...
	public double f(double x, double a, double b, double c) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x> 0) {
			res = a * (1 - Math.exp( - b * (1 - Math.exp( - c * x * x / 2))));
		}
		return res;
	}
	...
	public double dfa(double x, double a, double b, double c) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x > 0) {
			res = (1 - Math.exp( - b * (1 - Math.exp( - c * x * x / 2))));
		}
		return res;
	}
	...
	public double dfb(double x, double a, double b, double c) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x > 0) {
			res = a * (1 - Math.exp( - c * x * x / 2)) * Math.exp( - b * (1 - Math.exp( - c * x * x / 2)));
		}
		return res;
	}
	...
	public double dfc(double x, double a, double b, double c) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x > 0) {
			res = a * b * x * x * Math.exp( - c * x * x / 2) * Math.exp( - b * (1 - Math.exp( - c * x * x / 2))) / 2;
		}
		return res;
	}
	...
	public double dfd(double x, double a, double b, double c, double d) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x > 0) {
			res = - a * b * c * x * Math.exp( - b * (1 - Math.exp( - c * x * x / 2))) * Math.exp( - c * x * x / 2);
		}
		return res;
	}
