...
public class L_SRGMFunctionWithBC_bc implements ParametricUnivariateFunction  {

	private int n;

	public L_SRGMFunctionWithBC_bc(int nval) {
		n = nval;
	}

	public L_SRGMFunctionWithBC_bc() {
		n = 1;	
	}
...
/////// ============== edit this part
	...
	public double dfb(double x, double a, double b, double c) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x > 0) {
			res = - a * Math.exp( -c * x ) / ( (1 + b * Math.exp( -c * x ) ) * (1 + b * Math.exp( -c * x ) ) );
		}
		return res;
	}
	...
	public double dfc(double x, double a, double b, double c) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x > 0) {
			res = a * b * x * Math.exp( -c * x ) / ( (1 + b * Math.exp( -c * x ) ) * (1 + b * Math.exp( -c * x ) ) );
		}
		return res;
	}
	...
	public double dfd(double x, double a, double b, double c, double d) {
		double res = 0.0;// Double.MIN_VALUE;
		if (x > 0) {
			res = - a * b * c *  Math.exp( -c * x ) / ( (1 + b * Math.exp( -c * x ) ) * (1 + b * Math.exp( -c * x ) ) );
		}
		return res;
	}
